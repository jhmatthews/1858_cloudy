...
def make_figure(logxis, folder="cld_data/", TEMP=50.0, LOG_N=9.0, sed ="bb"):
    ...
    # get data
    cld_data = get_cloudy_line_strengths(TEMP, LOG_N, logxis, sed = sed, folder=folder)
    ...
    if plot_Mauche: plt.plot(lxi_mauche, si4ratio_mauche, **mauche_kwargs)
    ...
    if plot_Mauche: plt.plot(si4ratio_mauche, n5ratio_mauche, **mauche_kwargs)
    ...
    if plot_Mauche: plt.plot(si4ratio_mauche, he2ratio_mauche, **mauche_kwargs)
    plt.plot(logratios, logratios, ls="--", c="k")
    plt.xlim(-4, 4)
    plt.ylim(-4, 4)
